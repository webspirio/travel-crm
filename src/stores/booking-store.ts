...
const createBookingStoreSlice: StateCreator<BookingStore> = (set) => ({
  ...
  removePassenger: (localId) =>
    set((state) => {
      ...
      if (!target || target.isPrimary) {
        console.warn(
          "removePassenger: cannot remove primary passenger or unknown id",
          localId,
        )
        return state
      }
      return {
        passengers: state.passengers.filter((p) => p.localId !== localId),
      }
    }),
  ...
  reorderPassengers: (localIds) =>
    set((state) => {
      ...
      if (
        ...
        console.warn(
          "reorderPassengers: localIds set does not match current passengers",
        )
        return state
      }
      const ordered = localIds.map(
        (id) => state.passengers.find((p) => p.localId === id)!,
      )
  ...
  assignToRoom: (passengerLocalId, roomLocalId) =>
    set((state) => {
      const room = roomLocalId
        ? state.rooms.find((r) => r.localId === roomLocalId)
        : null
      return {
        passengers: state.passengers.map((p) =>
...
export const useBookingStore = create<BookingStore>()(
  persist(createBookingStoreSlice, {
      ...
      if (fromVersion < 2) {
        ...
        try {
          ...
          const nationality: "UA" | "DE" | null =
            nc?.nationality === "UA" || nc?.nationality === "DE"
              ? nc.nationality
              : null
          ...
          const primary: PassengerDraft = {
            ...
            roomGroupId: null,
            roomType:
              typeof p?.roomType === "string" ? (p.roomType as RoomType) : null,
            priceEur:
              typeof pricingRaw?.total === "number" ? pricingRaw.total : 0,
            priceOverridden: false,
          }
